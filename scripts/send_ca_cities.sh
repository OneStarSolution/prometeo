...
for instance_name in ${instance_names[@]}; do
    python3 scripts/split_file.py --chunks 20 --path CAN_city.csv --keep $counter
    echo $instance_name
    # Connect using SSH
    # gcloud compute ssh --project=directed-pier-294505 --zone=us-west2-a $instance_name --command='sudo rm instance_*.txt'
    gcloud compute scp --recurse --zone=us-west2-a cacities_tocrawl.csv $instance_name:~
    gcloud compute ssh --project=directed-pier-294505 --zone=us-west2-a $instance_name --command='sudo mv cacities*.csv prometeo/cacities.csv'
    let counter++
